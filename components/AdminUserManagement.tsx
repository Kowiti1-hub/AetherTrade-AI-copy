...
const AdminUserManagement: React.FC = () => {
  ...
  const deleteUser = (id: string) => {
    if (window.confirm("Permanent deletion of account? This cannot be undone.")) {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm">Monitor and control trader access to the floor.</p>
        </div>
        <button 
      ...
      {isAdding && (
        <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl animate-in zoom-in-95">
          ...
          <form onSubmit={handleAddUser} className="grid grid-cols-2 gap-4">
            ...
            <input type="text" placeholder="Country" value={newUser.country} onChange={e => setNewUser({...newUser, country: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm" />
            <div className="col-span-2 flex space-x-2">
              <button type="submit" className="flex-1 py-3 bg-emerald-600 rounded-xl font-bold">Provision Account</button>
            ...
            {users.map(u => (
              <tr key={u.id} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-[10px] text-slate-500">{u.country} • {u.phone}</div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end space-x-2">
                    <button 
                      ...
                      title={u.status === 'ACTIVE' ? 'Stop User' : 'Resume User'}
                    >
                      {u.status === 'ACTIVE' ? (
